...
function getFileName($hash) {
  return dirname(__FILE__) . "/data/$hash";
}
...
function savePlain($hash, $data) {
  if (!file_exists($dir = dirname( getFileName($hash) ))) mkdir($dir, 0777 , true);
  file_put_contents(getFileName($hash), $data);
  chmod(getFileName($hash), 0755);
}

function loadPlain($hash) {
  $data = @file_get_contents(getFileName($hash));
  return $data;
}
