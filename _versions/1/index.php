...
It is optimized to handle large number of indexes inside rapid search engine.
This demo does searching of md5 hash in up to four indexes at the same time and finds
intersections in them. That'all, no magic ;) <a class="more" href="more">Learn More</a></p>

<p class="desc" id="more" style="display: none">At the moment demonstration base is 
<?php $cur = unserialize(file_get_contents(getFileName('current'))); ?>
<?php echo round($cur * 100 / base2dec('ZZZZZ') * 100)/100?>% generated. It stores
<?php echo $cur ?> passwords 
[0-9a-zA-Z], up to <?php echo strlen(dec2base($cur))?> characters long. Last addition is '<?php echo dec2base($cur) ?>'.
There are <?php echo $count = count(unserialize(file_get_contents(getFileName('index')))) ?> index files, 
totally ~<?php $size = filesize(getFileName('0/aa')); echo round($count * $size / 1024 / 1024 *100)/100?> Mbytes.
...
function search($md)
{
	...
	while (1)
	{
	  ...
	  {
		foreach ($fs as $f) fclose($f);
		return( "$md decrypted is '" . dec2base($cur). "'" );
	  }
	  $fi++;
	  if (!isset ($fs[$fi])) $fi = 0;
	}
